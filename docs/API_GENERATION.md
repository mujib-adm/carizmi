...
public OpenAPI customOpenAPI() {
    return new OpenAPI()
        .info(new Info()
            .title("Carizmi Platform API — Auto-generated OpenAPI specification.")
            .version("1.0.0")
...
```yaml
- name: Build and Verify Backend (Contract Generation)
  run: mvn -B verify -pl backend -DskipTests
  # This runs the integration-test phase, which triggers OpenApiLocalGenerator
...
- name: Check for API Contract Drift
  run: |
    if ! git diff --exit-code frontend/src/api/openapi.json; then
      ...
      echo "::error::Please run 'mvn install' locally and commit the updated openapi.json."
      exit 1
    fi
```
